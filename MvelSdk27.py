...
try:
    ...
    reIntento     = 30
    USER_NAME  = getpass.getuser()#Nombre de usuario maquina
...
def conexionFTP():
    ...
    try:
        request = urllib2.urlopen('http://'+hostFtp)
        data = request.read()
        try:
            ftp = FTP_TLS()
...
def activarIntTFHKA():
    try:
        ...
        if estado_error()[0] != "128":
            ...
            U0Z        = open(Ru0z, "a+")
            final_de_U0Z = U0Z.tell()
            listaU0Z = ['%s \n'% str(reporteZ)]
            U0Z.writelines(listaU0Z)
            U0Z.seek(final_de_U0Z)
            U0Z.close()
            rS1          = open(Rs1, "a+")
            final_de_rS1 = rS1.tell()
            listarS1 = ['%s \n'% str(reporteS1)]
            rS1.writelines(listarS1)
            rS1.seek(final_de_rS1)
            rS1.close()
